...
export default function InviteEarn() {
  ...
  return (
    <div className="min-h-dvh bg-[#0B1426] text-white">
      <main className="mx-auto w-full max-w-[430px] px-4 pb-4 pt-5">
        <div className="mb-5 flex items-center justify-between">
          <Link to="/user/profile" className="text-sm font-semibold text-white/80 hover:text-white">
            ← Back
          </Link>
          <button
            type="button"
            className="inline-flex size-10 items-center justify-center rounded-xl border border-white/15 bg-white/10"
        ...
        <div className="text-center">
          <h1 className="font-heading text-[34px] font-extrabold tracking-[-0.02em]">Invite &amp; Earn</h1>
          <p className="mt-2 text-sm text-white/70">Bring trusted vendors to Gidira and get rewarded.</p>
        </div>
        ...
        {referralsQuery.isLoading ? (
          ...
          <div className="mt-6 space-y-4">
            <div className="flex items-center justify-between rounded-[18px] border border-white/12 bg-white/7 px-[18px] py-4 backdrop-blur-md">
              <div className="flex items-center gap-3">
                <span className="flex size-[34px] items-center justify-center rounded-[10px] bg-white/10">
                  <UserPlus className="size-[18px] text-[#E8C677]" />
                </span>
                <div>
                  <b className="font-heading text-[17px] font-bold">{inviteCount}</b>
                  <small className="block text-[11.5px] text-white/55">vendors invited</small>
                </div>
              </div>
            ...
            <section className="rounded-[22px] border border-white/12 bg-white/7 p-5 backdrop-blur-md">
              <div className="font-heading text-[40px] font-extrabold leading-none text-[#E8C677]">₦1,000 each</div>
              ...
              <span className="mt-3 inline-flex items-center gap-1.5 rounded-full border border-[#E8C677]/30 bg-[#E8C677]/14 px-3 py-1.5 text-xs font-bold text-[#F6D98B]">
                  ...
                  <li key={title} className="flex items-start gap-3 py-1.5">
                    <span
                      className={cn(
                        ...
                        index === 3
                          ? 'bg-gradient-to-br from-[#9A6B1F] to-[#C99A3F] text-white'
                          : 'bg-white/10 text-[#F6D98B]',
                      )}
                    >
                      {index === 3 ? '✓' : index + 1}
                    </span>
                    <div>
                      ...
                      <small className="text-[12.5px] text-white/55">{subtitle}</small>
                    </div>
                  </li>
                ))}
              </ol>
            ...
            <div className="flex items-center gap-3 rounded-2xl border border-white/12 bg-white/9 px-4 py-4">
              <div className="min-w-0 flex-1">
                <b className="block font-heading text-[23px] font-extrabold tracking-[0.18em]">{code}</b>
                <small className="text-[11.5px] tracking-[0.04em] text-white/55">YOUR UNIQUE REFERRAL CODE</small>
              </div>
              <button
                type="button"
              ...
              >
                <Copy className="size-5 text-[#F6D98B]" />
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <button
                type="button"
                className="inline-flex items-center justify-center gap-2 rounded-[13px] border border-white/12 bg-white/7 px-3 py-3 text-[13.5px] font-semibold"
                onClick={() => {
            ...
            <button
              type="button"
              className="flex w-full items-center justify-center gap-2 rounded-2xl bg-brand px-4 py-4 text-[16.5px] font-bold shadow-[0_12px_28px_rgba(225,36,42,0.4)]"
              onClick={() => void shareInvite()}
            >
            ...
            <div className="overflow-hidden rounded-2xl border border-white/12 bg-white/5">
              ...
              {termsOpen ? (
                <ul className="space-y-2 border-t border-white/10 px-4 py-4 text-[13px] leading-relaxed text-white/70">
            ...
            <p className="text-center text-[11.5px] leading-relaxed text-white/55">
              Rewards are paid in <span className="font-semibold text-[#F6D98B]">Gidira wallet credit</span>, not cash.
            </p>
          </div>
        )}
      </main>
