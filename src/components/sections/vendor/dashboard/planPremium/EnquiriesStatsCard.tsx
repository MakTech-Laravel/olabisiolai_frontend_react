...
export function EnquiriesStatsCard({ dashboard }: VendorDashboardCardProps) {
  const { stats } = dashboard;
  const delta = formatDashboardDelta(stats.enquiriesDeltaPercent);
      ...
      <CardContent className="space-y-4 p-6 md:p-8">
        ...
        <div>
          <p className="text-3xl font-bold tracking-tight text-foreground font-manrope">
            {formatDashboardCount(stats.enquiries)}
          </p>
        ...
        <div className="h-2 w-full rounded-full bg-muted overflow-hidden">
          <div
            ...
            style={{ width: `${stats.enquiriesProgressPercent}%` }}
          />
        </div>
      </CardContent>
    </Card>
