...
export function AppNotificationBell({ viewAllHref, triggerClassName }: AppNotificationBellProps) {
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          variant="ghost"
          className={cn(
            'relative h-10 w-10 rounded-xl p-0 text-foreground hover:bg-[#F1F5F9]',
            triggerClassName,
          )}
      ...
      <DropdownMenuContent align="end" className="w-[min(100vw-2rem,22rem)] p-0">
        ...
        >
          <Link to={viewAllHref}>View all notifications</Link>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  )
