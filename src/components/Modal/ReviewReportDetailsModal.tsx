...
import type { ReviewReportDto } from "@/features/reviewReports/types";
...
function ReportStatusBadge({ status }: { status: ReviewReportDto["status"] }) {
  const styles =
    status === "pending"
...
export function ReviewReportDetailsModal({
  open,
  onClose,
  report,
  processing = false,
    ...
    >
      ...
      >
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <h3 id="review-report-modal-title" className="text-base font-semibold text-ink-heading">
              Review Report
            </h3>
        ...
        <div className="space-y-4 text-sm">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-chat-meta">Reported review</p>
            <p className="mt-1 font-medium text-ink">{report.review?.reviewer_name ?? "—"}</p>
            {report.review?.rating != null ? (
              <p className="text-xs text-amber-600">{"★".repeat(report.review.rating)}</p>
            ) : null}
